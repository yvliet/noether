...
- **Non-Blocking Main Thread**: Heavy operations (text embeddings, graph physics, large PDF parsing) must be offloaded to Web Workers via `app.workerPool` or debounced so typing and navigation never freeze.
- **Fast Activation**: Extensions should activate quickly on startup. Dynamic imports should be used for heavy submodules. Read [[Optimizing Extension Load Time]].
