...
3. **Extension Registries**: Extensions plug into the application through dedicated registries:
   ...
   - `CanvasCardRegistry`: Provides modular card rendering for the infinite canvas via the `canvas:register-card-renderer` EventBus bridge, allowing extensions to supply custom card visualizations (Kanban boards, widgets, charts) without coupling to core canvas code.
